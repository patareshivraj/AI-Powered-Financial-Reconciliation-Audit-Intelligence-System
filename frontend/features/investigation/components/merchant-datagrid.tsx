...
export function MerchantDatagrid({ sessionId }: { sessionId: string }) {
  ...
  if (rows.length === 0) {
    return (
      <div className="w-full p-8 bg-black/40 border border-white/[0.05] rounded-2xl text-center shadow-inner">
        <Store className="h-8 w-8 text-neutral-600 mx-auto mb-4" />
        <p className="text-sm text-neutral-500 font-mono tracking-wide">NO MERCHANT DATA AVAILABLE FOR THIS SESSION.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="overflow-x-auto max-h-[calc(100vh-450px)] min-h-[300px] custom-scrollbar">
          <table className="w-full text-left text-xs border-collapse">
            <thead className="sticky top-0 z-10 bg-black/80 backdrop-blur-xl border-b border-white/[0.05]">
              <tr>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs">Merchant / Entity</th>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs text-right">Transactions</th>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs text-right">Total Volume</th>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs text-right">Mismatches</th>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs text-right">Risk Score</th>
                <th className="px-4 py-3 font-bold text-neutral-300 tracking-wider uppercase text-xs text-center">Action</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-4 py-3 border-b border-white/[0.02] text-right">
                      <span className={`px-2.5 py-1 rounded-md text-xs font-bold font-mono border shadow-sm ${riskBg} ${riskColor}`}>
                    ...
                    <td className="px-4 py-3 border-b border-white/[0.02] text-center">
                      <button 
                        ...
                        className="inline-flex items-center justify-center gap-2 min-h-[44px] px-4 py-2 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 transition-all text-xs font-bold uppercase tracking-wider mx-auto"
                      >
      ...
      {/* Deep Dive Modal */}
      {selectedMerchant && mounted && typeof document !== 'undefined' && createPortal(
            ...
            {/* Header */}
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-white tracking-tight">{selectedMerchant}</h2>
                    <p className="text-xs text-neutral-400 uppercase tracking-widest font-mono mt-1">Intelligence Deep Dive</p>
                  </div>
                </div>
                <button 
                  onClick={() => setSelectedMerchant(null)}
                  className="p-3 rounded-full hover:bg-white/10 transition-colors text-neutral-400 min-h-[44px] min-w-[44px] flex items-center justify-center"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
                    ...
                    <div className="rounded-xl border border-white/5 bg-black/40 overflow-hidden ring-1 ring-white/5">
                      <table className="w-full text-left text-xs border-collapse">
                        <thead className="bg-white/[0.02] border-b border-white/5">
                          <tr>
                            <th className="px-4 py-3 font-bold text-neutral-400 uppercase text-xs">Date</th>
                            <th className="px-4 py-3 font-bold text-neutral-400 uppercase text-xs">Ref ID</th>
                            <th className="px-4 py-3 font-bold text-neutral-400 uppercase text-xs text-right">Amount</th>
                            <th className="px-4 py-3 font-bold text-neutral-400 uppercase text-xs">Status</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                          {deepDiveData.transactions.map((tx: any, idx: number) => (
                            <tr key={idx} className="hover:bg-white/[0.02]">
                              <td className="px-4 py-3 text-neutral-300 font-mono">{new Date(tx.date).toLocaleDateString()}</td>
                              <td className="px-4 py-3 text-neutral-400 font-mono text-xs">{tx.reference}</td>
                              <td className="px-4 py-3 text-neutral-200 font-mono font-bold text-right">{fmt(tx.amount)}</td>
